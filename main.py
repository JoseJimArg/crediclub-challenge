...
async def read_payments(skip: int = 0, limit: int = 100, db: Session = Depends(utils.get_db)):
    payments = utils.get_payments(db, skip=skip, limit=limit)
    return payments

@app.get("/payments/download_csv", response_class=StreamingResponse)
async def read_payments_csv(skip: int = 0, limit: int = 100, db: Session = Depends(utils.get_db)):
    payments = utils.get_payments(db, skip=skip, limit=limit)
    # generate the file from the DataFrame
    payments_dataframe = utils.generate_payments_data_frame(payments=payments)
...
async def read_payments_total(skip: int = 0, limit: int = None, db: Session = Depends(utils.get_db)):
    payments = utils.get_payments(db, skip=skip, limit=limit)
    total = 0.0
...
async def read_payment(payment_id: int, db: Session = Depends(utils.get_db)):
...
def write_payment(payment: PaymentCreateSchema, db: Session = Depends(utils.get_db)):
